...
import time
import transformer_cross_attention_combine_nobag_double as model_test
import pickle
import sys
import os
#reload(sys)
...
#    os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    ...
    filestr = "../entity_type/data/entity_type.csv"
    ...
    for step in tqdm(range(steps-1)):
        ...
        # print(len(batch_relation))
        # print(len(batch_prob))
        # print(len(batch_entitypair))
        ...
        for i in range(len(batch_relation)):
            ...
            fw.write(str(batch_entitypair[i])+"|"+str(batch_prob[i])+"|"+str(batch_relation[i])+"|"+str(label_relations)+"|"+str(batch_test_date[i])+"\n")
...
if __name__ == '__main__':
    ...
    produce_pred_data(save_path='../entity_type/entityType_connect/model_0.2_0.2epoch1_new_.ckpt',output_path = '../entity_type/result/origin_pred_entitypair.pkl')
    ...
    Precision, Recall = PR_curve(label_path='../entity_type/data/label_entitypair.pkl', pred_path='../entity_type/result/origin_pred_entitypair.pkl',
                             num_total=num_total)
    print(Precision)
    print(Recall)
    List_Precision.append(Precision)
    ...
    np.save('../entity_type/data/List_Precision.npy',List_Precision)
    np.save('../entity_type/data/List_Recall.npy', List_Recall)
